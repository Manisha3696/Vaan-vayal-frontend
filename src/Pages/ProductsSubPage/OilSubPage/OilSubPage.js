            ...
            <div className="organic-shop container px-4 py-6">
                <div className="mt-5 mb-2 d-flex justify-content-start gap-4 align-item-center">
                    <img
                        ...
                        onClick={() => navigate(-1)}
                    />
                    <h3 style={{ color: "#2e7d32" }}>{oil.name}</h3>
                </div>
                <main className="main mb-5 mx-auto">
                    <div className="product-details-container" style={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
                        <div className="product-image" style={{ flex: '1 1 300px', maxWidth: '400px' }}>
                            <img src={oil.image} alt={oil.name} style={{ width: '100%', height: 'auto', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }} />
                        </div>
                        <div className="product-details" style={{ flex: '1 1 400px' }}>
                            <h3 style={{ color: '#2e7d32', marginBottom: '15px' }}>{oil.title}</h3>
                            <p className="price">{oil.priceRange}</p>
                            <p className="description" style={{ fontSize: '14px', lineHeight: '1.6', marginBottom: '20px' }} >{oil.description}</p>
                            <div className="categories" style={{ fontSize: '14px', color: '#777', marginBottom: '20px' }}>Categories: Best Deals, Organic Oil Exporters</div>
                        </div>
                    </div>
                </main>
            </div>
